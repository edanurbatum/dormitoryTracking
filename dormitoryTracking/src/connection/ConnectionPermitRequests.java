    ...
    public void populateTable(String sql,JTable tbl){
        ...
        try {
            ...
            for (PermitRequest permitRequest:permitRequests) {
                Object[]row={
                    permitRequest.getId(), permitRequest.getTc(),permitRequest.getName(),
                    permitRequest.getLastName(),permitRequest.getRemainingPermitAmount(),
                    permitRequest.getPermitStart(),permitRequest.getPermitFinish(),
                    permitRequest.getPermitAddress(),permitRequest.getUsedPermitAmount(),permitRequest.getResponse()
                }; 
                model.addRow(row);
            }     
        ...
        try{
            ...
            while(resultSet.next()){
                permitRequests.add(new PermitRequest(
                        ...
                        resultSet.getString("permitAddress"),
